...
export default function App() {
  ...
  const handleAnonymousLogin = () => {
    setUser({ uid: 'local-dev', displayName: '探索者', email: 'dev@local' } as User);
  };
      ...
      {!user ? (
        <div className="min-h-screen bg-[#0a0502] text-white flex flex-col items-center justify-center p-6 text-center">
          ...
          >
            ...
            <button
              onClick={handleLogin}
              className="px-8 py-3 bg-[#ff4e00] text-white rounded-full font-medium hover:bg-[#e64600] transition-colors shadow-lg shadow-[#ff4e00]/20"
            >
              开启灵感之旅 (Google 登录)
            </button>
            {location.hostname === 'localhost' && (
              <button
                ...
                className="mt-3 px-8 py-3 bg-white/10 text-gray-300 rounded-full font-medium hover:bg-white/20 transition-colors text-sm"
              >
                本地开发模式 (匿名登录)
              </button>
            )}
